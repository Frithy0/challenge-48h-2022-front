...
<template>
  ...
  <div class="container">
    <div >
      <input style="margin-right: auto;margin-left: auto;width: 400px;margin-bottom: 20px;" class="form-control" v-model="inputText" type="text" />
    </div>
    ...
    <div class="row">
      <div
        ...
        v-show="(item.name && item.name.toUpperCase().includes(inputText.toUpperCase())) || (item.title && item.title.toUpperCase().includes(inputText.toUpperCase()))"
      >
        <div class="card bg-dark mb-3 text-center">
          <div class="card-body text-warning ">
            ...
            <p class="card-text" v-if="item.display">
                ...
                <!--<li v-if ="$route.params.name =='people'" > {{ 'Homeworld : ' + item.homeworld.name}}</li> -->

                <li v-if ="$route.params.name =='planets'" > {{ 'Climate : ' + item.climate}}</li>
                <li v-if ="$route.params.name =='planets'" > {{ 'Diameter : ' + item.diameter}}</li>
                <li v-if ="$route.params.name =='planets'" > {{ 'Population : ' + item.population + " hab"}}</li>
                ...
                <li v-if ="$route.params.name =='species'" > {{ 'Categorie species : ' + item.classification}}</li>
                <li v-if ="$route.params.name =='species'" > {{ 'Height : ' + item.average_height + ' cm'}}</li>
                <li v-if ="$route.params.name =='species'" > {{ 'Lifespan : ' + item.average_lifespan + ' years'}}</li>
                
                <li v-if ="$route.params.name =='vehicles'" > {{ 'Capacity : ' + item.cargo_capacity + ' peoples'}}</li>
                ...
                <li v-if ="$route.params.name =='starships'" > {{ 'Cargo capacity : ' + item.cargo_capacity + " peoples" }}</li>
                ...
                <li v-if ="$route.params.name =='starships'" > {{ 'Number of passengers : ' + item.passengers + " peoples" }}</li>
                <li v-if ="$route.params.name =='starships'" > {{ 'Manufacturer : ' + item.manufacturer  }}</li>

            </p>
          </div>
          <div class="card-footer bg-transparent border-dark">
            <button v-if="!item.display" @click="item.display = true" class="btn btn-outline-warning">See More</button>
